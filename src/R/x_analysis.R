...
env_airflow_stratified <- env_sessions %>%
  ...
  dplyr::select(-col) %>%
  # Pivot wider: columns are session_sat x level combos
  tidyr::pivot_wider(
...
# 2. Thermal Sensation / Comfort / Preference Responses -----------------------------------


# Distribution of whole-body and ankle thermal perception by condition -> Boxplots / Stacked Bar Charts
# Stat: Test effect of SAT and air speed (conditions) on whole-body / ankle thermal sensation (mixed model or repeated measures)


# Row 1: THERMAL SENSATION

thermal_sensation_p <- survey %>%
  dplyr::filter(question %in% c("thermal_sensation", "thermal_sensation_ankles"),
                is_open_text == FALSE) %>%
  ...
  dplyr::mutate(
    ...
    question       = factor(
      ...
      levels = c("Overall", "Ankles")  # controls left-right order
    )
  ) %>%
  ggplot(aes(x = workstation, y = response_value, group = interaction(workstation, question))) +
  geom_boxplot(aes(fill = question), color = "grey70", outlier.shape = NA, alpha = 0.5,
               position = position_dodge(width = 0.8)) +
  geom_jitter(aes(color = response_value, group = question),
...
# Row 2: THERMAL PREFERENCE

# Plot

thermal_preference_overall_p <- survey %>% 
  dplyr::filter(question %in% c("thermal_preference"),
                is_open_text == FALSE) %>%
  dplyr::filter(workstation != "adaptation") %>%
  dplyr::mutate(
    response_value = as.numeric(response_value),
  ...
  plot_stacked_pct_ws(response_value, thermal_preference_palette)  + labs(subtitle = "Thermal preference  |  Overall", x = NULL)


thermal_preference_ankles_p <- survey %>% 
  dplyr::filter(question %in% c("thermal_preference_ankles"),
                is_open_text == FALSE) %>%
  dplyr::filter(workstation != "adaptation") %>%
  dplyr::mutate(
    response_value = as.numeric(response_value),
  ...
  plot_stacked_pct_ws(response_value, thermal_preference_palette) + labs(subtitle = "Thermal preference  |  Ankles", x = "Air speed (m/s)")
...
# Row 3: THERMAL ACCEPTABILITY
vertical_gap <- 0.2

thermal_acceptability_p <- survey %>%
  dplyr::filter(question %in% c("thermal_acceptability"),
                is_open_text == FALSE) %>%
  dplyr::filter(workstation != "adaptation") %>% 
  dplyr::mutate(
    response_value = as.numeric(response_value),
    response_plot  = ifelse(response_value > 0, response_value + vertical_gap, response_value)
  ) %>%
  ggplot(aes(x = workstation, y = response_plot)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 0.1 + vertical_gap, ymax = 3 + vertical_gap, fill = "#c1e0b9", alpha = 0.2) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -3,        ymax = -0.1,     fill = "#d99fa8", alpha = 0.2) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -0.1,      ymax = 0.1 + vertical_gap, fill = "white") +
  geom_hline(yintercept =  0.1 + vertical_gap, linetype = "dashed", color = "grey50", linewidth = 0.3) +
  geom_hline(yintercept = -0.1,       linetype = "dashed", color = "grey50", linewidth = 0.3) +
  ...
  scale_y_continuous(
    breaks = c(-3, -2, -1, -0.1, 0.1 + vertical_gap, 1 + vertical_gap, 2 + vertical_gap, 3 + vertical_gap),
    labels = c(
...
air_movement_acceptability_p <-survey %>%
  dplyr::filter(question %in% c("air_movement_acceptability_ankles"),
                is_open_text == FALSE) %>%
  dplyr::filter(workstation != "adaptation") %>%
  dplyr::mutate(
    response_value = as.numeric(response_value),
  ...
  plot_stacked_pct_ws(acceptability, palette = c("Unacceptable" = "#d99fa8", "Acceptable" = "#c1e0b9")) + labs(subtitle = "Ankle air movement acceptability", x = NULL)


air_movement_preference_p <- survey %>%
  dplyr::filter(question %in% c("air_movement_preference_ankles"),
                is_open_text == FALSE) %>%
  dplyr::filter(workstation != "adaptation") %>%
  dplyr::mutate(
    response_value = as.numeric(response_value),
  ...
  plot_stacked_pct_ws(acceptability, palette = air_movement_preference_palette) + labs(subtitle = "Ankle air movement preference", x = "Air speed (m/s)")
